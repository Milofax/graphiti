...
class BaseOpenAIClient(LLMClient):
    ...
    async def _generate_response(
        ...
        # Select reasoning/verbosity params based on model size
        if model_size == ModelSize.small:
            current_reasoning = self.small_reasoning
            current_verbosity = self.small_verbosity
        else:
            current_reasoning = self.reasoning
            current_verbosity = self.verbosity
