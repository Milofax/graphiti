...
class TestNodeRecordParser:
    ...
    def test_lightweight_parses_list_of_pairs(self):
        ...
        node = get_entity_node_from_record(record, GraphProvider.FALKORDB, lightweight=True)
...
# ──────────────────────────────────────────────
# Integration Tests: FalkorDB round-trip
# ──────────────────────────────────────────────


@pytest.mark.asyncio
async def test_node_lightweight_roundtrip(graph_driver):
    """Save a node with embeddings, retrieve lightweight — embeddings should be None."""
    ...
    # Retrieve with lightweight=True
    nodes = await EntityNode.get_by_group_ids(
        graph_driver, [group_id], lightweight=True
    )
    assert len(nodes) >= 1
...
async def test_node_non_lightweight_still_works(graph_driver):
    ...
    nodes = await EntityNode.get_by_group_ids(
        graph_driver, [group_id], lightweight=False
    )
    retrieved = next(n for n in nodes if n.uuid == node.uuid)
...
async def test_edge_lightweight_roundtrip(graph_driver):
    """Save an edge with fact_embedding, retrieve lightweight — embedding should be None."""
    embedding = np.random.uniform(0.0, 0.9, embedding_dim).tolist()
